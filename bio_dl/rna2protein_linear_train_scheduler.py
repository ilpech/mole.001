# python3 bio_dl/rna2protein_linear_train_scheduler.py --config config/gene_expression/train_linear_cohorts.yaml --isdebug False 
import os
from torch_dl.train.train_gene_regression import DistGeneExpressionTrainer
from torch_dl.train.train_linear_gene_regression import DistGeneLinearExpressionTrainer
...
class ModelsCohort:
    ...
    def cohortTrain(self):
        ...
        # debug(cohort_chunks.shape)
        debug(len(train_genes))
        debug(len(cohort_chunks))
        debug([len(x) for x in cohort_chunks])
        debug(queue)
